...
[[ -f "$DEPLOY_KEY_PATH" ]] || { echo "missing deploy key: $DEPLOY_KEY_PATH" >&2; exit 2; }
...
if [[ -z "$(git status --porcelain=v1)" ]]; then
  echo "[gitops] clean; nothing to commit."; popd >/dev/null; exit 0
fi

git add -A
[[ -n "$(git status --porcelain=v1)" ]] || { echo "[gitops] no staged changes"; popd >/dev/null; exit 0; }
TS="$(date -Is)"
