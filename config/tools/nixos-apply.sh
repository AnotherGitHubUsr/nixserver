...
usage() {
  cat <<'USAGE'
Usage: nixos-apply.sh [--flake PATH] [--host NAME] [--impure]
...
[[ -r "$FLAKE/flake.nix" ]] || { echo "No flake.nix at $FLAKE" >&2; exit 64; }
...
if (( DO_MIRROR == 1 )); then
  ...
  [[ -n "${RSYNC_EXCLUDES:-}" ]] && read -r -a ADD <<<"$RSYNC_EXCLUDES" && EXCLUDES+=("${ADD[@]}")

  install -d -m 0755 "$MIRROR_TARGET"
  rsync -a --delete "${EXCLUDES[@]}" "$FLAKE"/ "$MIRROR_TARGET/" \
    || { echo "Mirror failed; backup in $BACKUP_DIR" >&2; exit 3; }
fi
...
if (( DO_EXPORT == 1 )); then
  ...
  if [[ -x "$EXPORT" ]]; then
    ...
    REL_ID="$("$EXPORT")" || { echo "Export failed." >&2; exit 4; }
    echo "Release: $REL_ID"
  else
...
if (( DO_GIT == 1 )); then
  ...
  if [[ -x "$AUTOCOMMIT" ]]; then
    ...
    "$AUTOCOMMIT" ${REL_ID:+--id "$REL_ID"} || { echo "Git commit/push failed." >&2; exit 5; }
  else
